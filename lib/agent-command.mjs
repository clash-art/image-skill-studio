...
function skillSnapshot(skill) {
  const hash = skill.contentHash ? `（内容哈希 ${skill.contentHash}）` : "";
  return `\`$${skill.id}\` Skill。先完整读取精确快照 ${skill.skillPath}${hash}`;
}
...
export function buildGenerationPrompt({
  ...
  const referenceText = references.length
    ? references
        ...
        .join("\n")
    : "无参考图。";
  const imagegen = "出图必须使用 `$imagegen` Skill，按它的默认内置工具模式，用内置 image_gen 工具生成 1 张最终图片。不要使用 CLI（scripts/image_gen.py），不要要求 OPENAI_API_KEY。";
  const method = isImagegenSkill(skill)
    ? imagegen
    : `必须同时使用两个 Skill：\n` +
      `1. 创作方法：${skillSnapshot(skill)}，严格遵循它的创作方法。\n` +
      `2. ${imagegen}`;
  const completion = transport === "mcp-app"
    ? `只有生成成功、并且已经有真实图片文件时，才调用本插件的 record_image_generation，传入 runId "${runId}"、status "succeeded"；优先把 ImageGen 返回的实际绝对路径作为 savedPath 传入，如果宿主只提供文件对象，则把产物作为 image 文件参数传入。生成失败时不要调用这个工具，不要记录失败。`
  ...
  return `你正在执行 Image Skill Studio 的一次图片生成任务。\n\n` +
    ...
    `参考图：\n${referenceText}\n\n` +
    `${completion}\n` +
    `不要修改项目源代码，不要改写用户提示词，不要用其他生图工具代替 \`$imagegen\`。`;
}
